<template>
	<view class="pages column">
		...
		<input class="inp" type="text" value="" placeholder="请输入验证码" placeholder-class="placeholder" @input="editCode"/>
...
<script>
	export default {
		...
		methods: {
			next () {
				if (this.trueCode) {
					this.$axios({ url: '',data: { tel:'', code:'' } }).then(res =>{
						console.log(res)
					})
					uni.switchTab({
			...
			editCode (e) { // 输入验证码时
				var reg = new RegExp('^(-[1-9]|[1-9])[0-9]{5}$');
				if((reg.test(e.detail.value))){
					this.trueCode = true
				}
			},
			getCode (tel) { // 获取验证码
				this.$axios({ url: '', data: { tel: tel } }).then(res =>{
